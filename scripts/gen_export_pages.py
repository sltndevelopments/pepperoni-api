...
COMMON_FACTS = {
    "ru": ["Условия поставки: EXW Казань (Incoterms 2020); иные условия — по согласованию.",
           ...
           "Ассортимент: живой каталог халяль — пепперони, сосиски, котлеты, копчёные колбасы, казылык, татарская выпечка.",
           "Private Label / СТМ: производство под маркой заказчика, объём — по договору."],
    "en": ["Delivery terms: EXW Kazan (Incoterms 2020); other terms negotiable.",
           ...
           "Range: live halal catalog — pepperoni, sausages, burger patties, smoked sausages, kazylyk, Tatar pastry.",
           "Private Label / OEM: production under the customer's brand; volume agreed per contract."],
    "ar": ["شروط التسليم: EXW قازان (إنكوتيرمز 2020)؛ وتُتاح شروط أخرى بالاتفاق.",
           ...
           "التشكيلة: الكتالوج الحي للحلال — بيبروني، نقانق، أقراص برغر، نقانق مدخنة، قازيليك، معجنات تتارية.",
           "العلامة الخاصة / OEM: الإنتاج تحت علامة العميل؛ الحجم باتفاق العقد."],
}
...
татарская выпечка (эчпочмак, чак-чак). B2B: опт, дистрибуция, HoReCa, Private Label/СТМ (объём по договору).
...
4 FAQ — практические вопросы импортёра (объём по договорённости / logistics, документы, сроки, Private Label).
Не указывай минимальный заказ в кг (100 кг, 300 кг, 500 кг, from 100 kg и т.п.) —
объём всегда по договорённости, зависит от логистики. Без вопросов о сертификатах."""


# Structured output: the API guarantees the reply matches this schema, so no
# more retries on malformed/truncated JSON.
GEN_SCHEMA = {
    "type": "object",
    "properties": {
        "title": {"type": "string"},
        ...
        "market_html": {"type": "string"},
        "logistics_html": {"type": "string"},
        "faq": {
            "type": "array",
            "items": {
                "type": "object",
    ...
    "required": ["title", "meta_description", "h1", "tagline",
                 "intro_html", "market_html", "logistics_html", "faq"],
}
    ...
    for c in COUNTRIES:
        ...
        for lang in c["langs"]:
            ...
            try:
                ...
                # auto-fallback to plain call when unavailable) + guaranteed
                # JSON via structured output.
