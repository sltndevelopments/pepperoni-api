...
.product-card__media--placeholder{{background:linear-gradient(165deg,#f0f4f0 0%,#e8ebe8 48%,#f0f4f0 100%);display:flex;align-items:center;justify-content:center;aspect-ratio:4/3;min-height:8rem}}
...
.product-card__logo{{width:46%;max-width:8rem;height:auto;object-fit:contain;opacity:.85}}
...
<div class="container">
  <div class="info-block" style="background:#f5f5ff;border-color:#0066cc">
    ...
    <a href="/api/export?lang=ru&amp;currency=RUB&amp;format=xlsx&amp;vat=incl" onclick="event.preventDefault();downloadFile('xlsx')" class="btn-dl btn-dl-solid">📊 Скачать Excel (.xlsx)</a>
    <a href="/api/export?lang=ru&amp;currency=RUB&amp;format=csv&amp;vat=incl" onclick="event.preventDefault();downloadFile('csv')" class="btn-dl btn-dl-solid">📄 Скачать CSV</a>
    <script>
  ...
  for(const{{section,icon,groups}}of allGroups){{
    ...
    for(const[cat,products]of Object.entries(groups)){{
      ...
      for(const p of products){{
        ...
        const props=[];
        if(p.weight&&!looksLikePrice(p.weight))props.push(`<span class="prop" title="Вес">${{fmtWeight(p.weight)}}</span>`);
        ...
        const rawImg=(p.image || p.imageMain || p.imagePack || p.imageSlice || '').trim();
        const hasImg=!!rawImg;
        const imgUrl=hasImg?catalogImgDisplayUrl(rawImg):'';
        const proxyUrl=hasImg?catalogImgProxyUrl(imgUrl):'';
        const imgForMeta=escAttr(rawImg||'/images/logo.png');
        ...
        const mediaBlock=hasImg
          ...
          :`<div class="product-card__media product-card__media--placeholder" role="img" aria-label="Фото товара скоро появится"><img src="/images/logo.png" alt="Казанские Деликатесы" class="product-card__logo" width="128" height="128" loading="lazy" decoding="async"></div>`;
...
(function registerWebMCPTools(){{
  const mc=(document.modelContext&&document.modelContext.registerTool)?document.modelContext
    :(navigator.modelContext&&navigator.modelContext.registerTool)?navigator.modelContext:null;
  if(!mc)return;
  const CURS=['RUB','USD','KZT','UZS','KGS','BYN','AZN'];
  mc.registerTool({{
    name:'set_catalog_currency',
    description:'Switch the displayed currency (and VAT mode, for RUB) of the halal meat/pastry product catalog on this page. Use this before quoting prices to a buyer in a specific currency.',
    inputSchema:{{type:'object',properties:{{
      currency:{{type:'string',enum:CURS,description:'ISO-like currency code to display prices in.'}},
      vat_included:{{type:'boolean',description:'Only applies when currency is RUB. If omitted, defaults to VAT included for RUB, excluded otherwise.'}}
    }},required:['currency']}},
    annotations:{{readOnlyHint:true,destructiveHint:false}},
    execute:async({{currency,vat_included}})=>{{
      if(!CURS.includes(currency))return `Unknown currency: ${{currency}}`;
      setCur(currency);
      if(typeof vat_included==='boolean'&&typeof setVAT==='function')setVAT(vat_included);
      return `Catalog now shows prices in ${{currency}}${{currency==='RUB'?(VAT?' (VAT included)':' (VAT excluded)'):''}}.`;
    }}
  }});
  mc.registerTool({{
    ...
    inputSchema:{{type:'object',properties:{{
      ...
      vat_included:{{type:'boolean',description:'Only applies when currency is RUB.'}},
      format:{{type:'string',enum:['xlsx','csv'],description:'File format.'}}
    }},required:['format']}},
    annotations:{{readOnlyHint:false,destructiveHint:false}},
    execute:async({{language,currency,vat_included,format}})=>{{
      const lang=(language==='en')?'en':'ru';
      const cur=CURS.includes(currency)?currency:CUR;
      const vat=(typeof vat_included==='boolean')?vat_included:VAT;
      const vatParam=cur==='RUB'?`&vat=${{vat}}`:'';
      window.location.href=`/api/export?lang=${{encodeURIComponent(lang)}}&currency=${{encodeURIComponent(cur)}}&format=${{format}}${{vatParam}}`;
      return `Started download: price list (${{lang}}, ${{cur}}, ${{format}}).`;
    }}
  }});
  mc.registerTool({{
    name:'contact_via_whatsapp',
    description:'Open a WhatsApp chat with Kazan Delicacies sales (halal pepperoni, sausages, burger patties, Tatar pastries manufacturer in Kazan, Russia), optionally with a pre-filled message describing what the buyer needs (product, quantity, destination).',
    inputSchema:{{type:'object',properties:{{
      message:{{type:'string',description:'Pre-filled message text, e.g. product and quantity needed.'}}
    }}}},
    annotations:{{readOnlyHint:false,destructiveHint:false}},
    execute:async({{message}})=>{{
      const base='https://wa.me/79872170202';
      const url=message?`${{base}}?text=${{encodeURIComponent(message)}}`:base;
      window.open(url,'_blank');
      return 'Opened WhatsApp chat with Kazan Delicacies sales.';
    }}
  }});
}})();
...
function render(){{
  ...
  var c=document.getElementById('catalog-inner');
  // Phase 1 — header + count: instant paint (FCP)
  c.innerHTML=renderControls()
    +`<div style="font-size:.83rem;color:#595959;margin-bottom:8px">${{total}} товаров · актуальные цены</div>`;
  // Phase 2 — product grid: deferred to avoid long task blocking first paint
  requestAnimationFrame(function(){{
    c.insertAdjacentHTML('beforeend',renderGroups(ALL_GROUPS));
  }});
}}
...
async function loadCatalog(){{
  try{{
    ...
    for(const p of d.products||[]){{
      ...
      const pnv=parseFloat(p.offers?.priceExclVAT||p.offers?.pricePerUnitExclVAT||0);
...
.seg-card{{display:flex;flex-direction:column;background:#fff;border:1px solid var(--border);border-radius:var(--radius);padding:20px;text-decoration:none;color:inherit;transition:box-shadow .2s,border-color .2s,transform .2s;position:relative;overflow:hidden}}
.seg-card:hover{{box-shadow:var(--shadow);border-color:var(--green);transform:translateY(-2px)}}
.seg-card__num{{position:absolute;top:14px;right:16px;font-size:.72rem;font-weight:700;color:#777;letter-spacing:1px}}
.seg-card__icon{{font-size:1.9rem;margin-bottom:8px}}
.seg-card__title{{font-weight:700;font-size:1rem;margin-bottom:6px;color:#1a1a1a}}
.seg-card__desc{{color:var(--muted);font-size:.85rem;line-height:1.5;margin-bottom:10px;flex-grow:1}}
.seg-card__case{{font-size:.78rem;color:var(--green);font-weight:600;margin-top:auto;padding-top:8px;border-top:1px dashed #eee}}
.seg-card__arrow{{display:inline-block;margin-left:4px;transition:transform .2s}}
.seg-card:hover .seg-card__arrow{{transform:translateX(3px)}}
.endpoint-list{{list-style:none;font-size:.85rem}}
.endpoint-list li{{padding:5px 0;border-bottom:1px solid var(--border)}}
.endpoint-list a{{color:#0066cc}}
.live-badge{{display:inline-block;background:#1b7a3d;color:#fff;font-size:.65rem;font-weight:700;padding:1px 6px;border-radius:3px;margin-left:6px;vertical-align:middle}}
...
<nav class="nav">
  <div class="container nav__inner">
    ...
    <div class="nav__links">
      <a href="/en/pepperoni">Pepperoni</a>
      <a href="/en/halal">Halal</a>
      <a href="/en/kazylyk">Kazylyk</a>
...
<div class="container">
  <div class="info-block dl-price-block" style="background:#f5f5ff;border-color:#0066cc">
    ...
    <a href="/api/export?lang=en&amp;currency=RUB&amp;format=xlsx&amp;vat=incl" onclick="event.preventDefault();downloadFile('xlsx')" class="btn-dl btn-dl-solid">📊 Download Excel (.xlsx)</a>
    <a href="/api/export?lang=en&amp;currency=RUB&amp;format=csv&amp;vat=incl" onclick="event.preventDefault();downloadFile('csv')" class="btn-dl btn-dl-solid">📄 Download CSV</a>
    <script>
      document.getElementById('dl-cur').addEventListener('change',function(){{document.getElementById('dl-vat-wrap').style.display=this.value==='RUB'?'':'none';}});
      function downloadFile(fmt){{const lang=document.getElementById('dl-lang').value,cur=document.getElementById('dl-cur').value,vat=document.getElementById('dl-vat').value,vatParam=cur==='RUB'?`&vat=${{vat}}`:'';window.location.href=`/api/export?lang=${{encodeURIComponent(lang)}}&currency=${{encodeURIComponent(cur)}}&format=${{fmt}}${{vatParam}}`;}}
    </script>
  ...
  <div class="info-block">
    ...
    <ul class="endpoint-list" style="margin-top:12px">
      ...
      <li><a href="/en/llms.txt">/en/llms.txt</a><span class="endpoint-desc"> — LLM instructions (English)</span></li>
      <li><a href="/sitemap.xml">/sitemap.xml</a><span class="endpoint-desc"> — sitemap</span></li>
    </ul>
  </div>
</div>
</main>
  ...
  for(const{{section,icon,groups}}of allGroups){{
    ...
    for(const[cat,products]of Object.entries(groups)){{
      ...
      for(const p of products){{
        ...
        const priceLabel=p.isBakery
          ?`${{fmtPrice(pr)}} ${{sym}}/pc`
          :`${{fmtPrice(pr)}} ${{sym}} <span class="product-price-vat">excl. VAT</span>`;
        const props=[];
        ...
        const mediaBlock=hasImg
          ?`<div class="product-card__media product-card__media--photo"><img src="${{escAttr(imgUrl)}}" data-proxy="${{escAttr(proxyUrl)}}" alt="${{altText}}" loading="${{eager?'eager':'lazy'}}" fetchpriority="${{eager?'high':'low'}}" width="400" height="300" decoding="async" onerror="if(this.dataset.proxy){{this.onerror=null;this.src=this.dataset.proxy}}"></div>`
          :`<div class="product-card__media product-card__media--placeholder" role="img" aria-label="Product photo coming soon"><img src="/images/logo.png" alt="Kazan Delicacies" class="product-card__logo" width="128" height="128" loading="lazy" decoding="async"></div>`;
        html+=`<a href="${{href}}" class="product-card-link"><div class="product-card product-card--featured" itemscope itemtype="https://schema.org/Product">
          <meta itemprop="sku" content="${{p.sku||''}}">
          <meta itemprop="image" content="${{imgForMeta}}">
...
(function registerWebMCPTools(){{
  const mc=(document.modelContext&&document.modelContext.registerTool)?document.modelContext
    :(navigator.modelContext&&navigator.modelContext.registerTool)?navigator.modelContext:null;
  if(!mc)return;
  const CURS=['USD','RUB','KZT','UZS','KGS','BYN','AZN'];
  mc.registerTool({{
    name:'set_catalog_currency',
    description:'Switch the displayed currency of the halal meat/pastry product catalog on this page (VAT is automatically included for RUB, excluded otherwise). Use this before quoting prices to a buyer in a specific currency.',
    inputSchema:{{type:'object',properties:{{
      currency:{{type:'string',enum:CURS,description:'Currency code to display prices in.'}}
    }},required:['currency']}},
    annotations:{{readOnlyHint:true,destructiveHint:false}},
    execute:async({{currency}})=>{{
      if(!CURS.includes(currency))return `Unknown currency: ${{currency}}`;
      setCur(currency);
      return `Catalog now shows prices in ${{currency}}${{currency==='RUB'?' (VAT included)':' (VAT excluded)'}}.`;
    }}
  }});
  mc.registerTool({{
    name:'download_price_list',
...
const SEC_EN={{'Заморозка':'Frozen Products','Охлаждённая продукция':'Chilled Products','Выпечка':'Bakery & Pastry'}};
const CAT_EN={{
  'Сосиски гриль для хот-догов':'Grill & Hot Dog Sausages',
  'Котлеты для бургеров':'Burger Patties',
  'Мясные заготовки':'Meat Semi-Finished',
  'Топпинги':'Toppings & Pepperoni',
  'Вареные':'Cooked Sausages',
  'Ветчины':'Ham & Delicacies',
  'Копченые':'Smoked Products',
  'Премиум Казылык':'Premium Kazylyk',
  'Сосиски, сардельки':'Sausages & Franks',
  'Классическая выпечка':'Classic Pastry',
...
const NAME_EN_OVERRIDES={{
  ...
  'Филе бедра куриного в кубике 1х1 см':'Diced Chicken Thigh 1×1cm',
  'Филе грудки куриной в кубике 1х1 см':'Diced Chicken Breast 1×1cm',
  'Говядина 1 сорт в кубике 1х1 см':'Diced Beef Grade 1, 1×1cm',
  ...
  'в/к Рамазан (половинка)':'Ramazan Smoked (half)',
  'в/к Мраморная':'Marbled Smoked Sausage',
  'в/к Мраморная (половинка)':'Marbled Smoked (half)',
  'в/к Филейный':'Fillet Smoked Sausage',
  'в/к Филейный (половинка)':'Fillet Smoked (half)',
  'в/к Княжеская':'Knyazheskaya Smoked Sausage',
  'в/к Княжеская (половинка)':'Knyazheskaya Smoked (half)',
  'Казылык «Премиум» в подарочной упаковке':'Kazylyk Premium (gift box)',
  'Казылык «Премиум» в нарезке в подарочной упаковке':'Kazylyk Premium Sliced (gift box)',
  'Губадия с кортом':'Gubadiya with Kort',
  'Чебурек жареный':'Fried Cheburek',
  'Перемяч жареный':'Fried Peremyach',
  'Самса с курицей':'Chicken Samsa',
  'Эчпочмак с говядиной и картофелем':'Echpochmak (beef & potato)',
  'Самса с говядиной':'Beef Samsa',
  'Элеш с курицей и картофелем':'Elesh (chicken & potato)',
  ...
  'Чак-Чак в пластиковой упаковке':'Chak-Chak (plastic box)',
  'Чак-Чак в крафтовой подарочной упаковке':'Chak-Chak (kraft gift box)',
...
async function loadCatalog(){{
  try{{
    ...
    const groups={{}};
    for(const p of d.products||[]){{
      const secRu=p.section||'Other',catRu=p.category||secRu;
      const sec=SEC_EN[secRu]||secRu,cat=CAT_EN[catRu]||catRu;
      if(!groups[sec])groups[sec]={{}};if(!groups[sec][cat])groups[sec][cat]=[];
      const pr=parseFloat(p.offers?.price||p.offers?.pricePerUnit||0);
      const pnv=parseFloat(p.offers?.priceExclVAT||p.offers?.pricePerUnitExclVAT||0);
      const isBakery=!!p.offers?.pricePerUnit;
