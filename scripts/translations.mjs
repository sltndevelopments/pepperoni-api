/**
 * Product translations RU→EN. Shared by sync-sheets and API.
 * Keys: lowercase product/category names for flexible matching.
 */
export const TRANSLATIONS = {
  ...
  'пепперони вар-коп из конины': 'Cooked-Smoked Pepperoni (horse meat)',
  'пепперони варено-копченый из конины': 'Cooked-Smoked Pepperoni (horse meat)',
  'пепперони варено-копченый куриный': 'Cooked-Smoked Chicken Pepperoni (sliced)',
  'пепперони варено-копченый куриный целый батон': 'Cooked-Smoked Chicken Pepperoni (whole stick)',
...
/** Returns product with name_en, category_en, section_en, etc. for EN pages */
export function translateProduct(product) {
  const key = product.name.toLowerCase().trim();
  const t = {
    name_en: TRANSLATIONS[key] || product.name,
    category_en: (product.category && CATEGORY_TRANSLATIONS[product.category]) || product.category,
    section_en: (product.section && SECTION_TRANSLATIONS[product.section]) || product.section,
    shelfLife_en: product.shelfLife ? (SHELF_TR[product.shelfLife] || product.shelfLife.replace('суток', 'days')) : '',
    storage_en: product.storage ? (STORAGE_TR[product.storage] || product.storage) : '',
    weight_en: product.weight ? product.weight.replace(',', '.').replace(' г', ' g').replace(' кг', ' kg') : '',
  };
  return t;
}
