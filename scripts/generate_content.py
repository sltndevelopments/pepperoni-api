...
def strip_model_junk_prefix(html: str) -> str:
    ...
    As a secondary pass it also strips any text that appears before the
    very first ``<`` character in the output — this catches cases where
    the model emits a preamble that contains no recognisable HTML marker
    at all (e.g. plain-text sentences followed by a bare ``<html>`` tag
    without a doctype).
    ...
    """
    # Primary pass: look for <!DOCTYPE or <html
    ...
    # Secondary pass: strip any non-HTML text that precedes the first '<'.
    # This handles outputs like "Вот страница:\n<html>…" where there is no
    # doctype but the HTML tag is still present.
    first_lt = html.find('<')
    if first_lt > 0:
        stripped = html[first_lt:]
        print(
            f"  ⚠️  stripped {first_lt} junk chars before first '<' "
            f"(prefix: {html[:60]!r}…)",
            file=sys.stderr,
        )
        return stripped
...
def is_valid_page(html: str) -> bool:
    ...
    The check is performed on the *stripped* version of the document so that
    leading whitespace does not cause false negatives.
    """
    if any(m in html for m in ("<<<<<<<", "=======\n", ">>>>>>>")):
        return False
    # Apply junk-prefix stripping before validating the opening tag so that
    # any residual preamble that survived earlier passes is also removed here.
    cleaned = strip_model_junk_prefix(html)
    # Validate that the document starts with a proper HTML tag.
    if not _VALID_HTML_START_RE.match(cleaned.lstrip()):
        return False
    low = cleaned.lower()
