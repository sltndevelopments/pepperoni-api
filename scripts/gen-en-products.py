...
def main():
    ...
    for p in products:
        ...
        for label, url, full in [("Pack", pack_img, pack_full), ("Slice", slice_img, slice_full)]:
            if url:
                thumbs.append(f'<span class="lightbox-trigger" data-full="{full}" tabindex="0" role="button"><img src="{url}" alt="{name_esc} — {label}" class="{img_class}" style="{img_style};max-height:120px" loading="lazy"/></span>')
