...
CERT_BLOCK_RU = (
    ...
    "ТР ТС 021/2011. Ветеринарные свидетельства РФ. "
    "Состав — только из карточки SKU, без заводских claim «без нитрита». Private Label / СТМ.**"
)
CERT_BLOCK_EN = (
    "**Certifications: Halal (DUM RT #614A/2024), HACCP, ISO 22000:2018, "
    "TR CU 021/2011. Russian veterinary certificates. "
    "Ingredients only from the SKU card — do not claim nitrite-free. Private Label available.**"
)
CERT_BLOCK_AR = (
    "**شهادات: حلال (DUM RT رقم 614A/2024)، HACCP، ISO 22000:2018. "
    "المكوّنات من بطاقة SKU فقط — لا تدّعِ خلو المنتج من النيتريت. الإنتاج بالعلامة التجارية الخاصة.**"
)
...
CERT_BLOCK_RU_IN_PROCESS = (
    ...
    "ISO 22000:2018, ТР ТС 021/2011. Состав — из карточки SKU. "
...
CERT_BLOCK_EN_IN_PROCESS = (
    ...
    "Ingredients from the SKU card only. Do not claim nitrite-free. "
...
TEMPLATE_PROMPTS = {
    ...
    "C": "Акцент на оптовые поставки: объём по договорённости (зависит от логистики), условия доставки, отсрочка платежа, дистрибьюторские условия.",
    ...
    "E": "Акцент на Private Label / СТМ: производство под брендом клиента, разработка рецептуры, дизайн упаковки, объём по договорённости.",
    "F": "Акцент на проверяемый состав: только ингредиенты из карточки SKU, без заводских claim про нитрит, сертификат ДУМ РТ.",
}
...
3. Структура (СТРОГО В ЭТОМ ПОРЯДКЕ):
   ...
   - БЛОК ОТВЕТ-ПЕРВЫМ (ОБЯЗАТЕЛЬНЫЙ, сразу после H1, ДО вступления):
     ...
     халяль-статус (сертификат), готовность к СТМ/OEM, объём по договорённости,
     ...
     Поставка в [Город] за [срок]. Объём — по договорённости, зависит от логистики. СТМ/OEM — да.»
...
5. Стиль: профессиональный, B2B, акцент на надёжность и сертификаты.
   ...
   ЗАПРЕЩЕНО указывать минимальный заказ в килограммах (100 кг, 300 кг, 500 кг,
   from 100 kg, MOQ 100, من 100 كجم и любые другие выдуманные кг-пороги). Объём всегда
   по договорённости, зависит от логистики. Не выдумывай новые цифры кг.
   ...
   «Халяль-сертификат ДУМ РТ», «Состав из карточки SKU», «Формат для пиццерий».
...
def build_user_prompt(product: dict, city_name: str, city_context: dict,
    ...
    random_angle = random.choice([
        "технология и состав: только факты из карточки SKU — без выдуманных claim",
