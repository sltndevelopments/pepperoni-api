#!/usr/bin/env bash
# DNS-AID for rinatsultan.com. Requires CLOUDFLARE_API_TOKEN or
# CLOUDFLARE_EMAIL+CLOUDFLARE_API_KEY. DNS-only (proxied=false).
set -euo pipefail
...
MCP='alpn="h2,h3" port=443'

upsert_svcb "_index._agents.$DOMAIN" "$DOMAIN" "$INDEX" HTTPS
upsert_svcb "_index._agents.$DOMAIN" "$DOMAIN" "$INDEX" SVCB
upsert_svcb "_mcp._agents.$DOMAIN" "$DOMAIN" "$MCP" HTTPS
upsert_svcb "_mcp._agents.$DOMAIN" "$DOMAIN" "$MCP" SVCB
upsert_svcb "_a2a._agents.$DOMAIN" "$DOMAIN" "$MCP" HTTPS
upsert_svcb "_a2a._agents.$DOMAIN" "$DOMAIN" "$MCP" SVCB
upsert_txt "_index._agents.$DOMAIN" "url=https://rinatsultan.com/.well-known/ai-catalog.json"
upsert_txt "_catalog._agents.$DOMAIN" "url=https://rinatsultan.com/.well-known/ai-catalog.json"
