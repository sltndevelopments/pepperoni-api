...
GENERATED_REDIRECT_CONFS = {
    "geo-cleanup-redirects.conf", "en-geo-cleanup-redirects.conf",
    "pepperoni-blog-redirects.conf", "trust-reset-redirects.conf",
}


def old_redirects(*, exclude_generated: bool = False) -> dict[str, str]:
    out: dict[str, str] = {}
    for conf in sorted(NGINX.glob("*redirects.conf")):
        if exclude_generated and conf.name in GENERATED_REDIRECT_CONFS:
            continue
...
def render_locations(rows: list[dict], status: str, header: str) -> str:
    ...
    for row in sorted(rows, key=lambda r: r["url"]):
        ...
        if source != "/" and not source.endswith(".html"):
            ...
            # GSC still reports the slash form of retired geo URLs; without an
            # exact match it falls into the ^~ /geo/ prefix and returns 410.
...
def main() -> int:
    ...
    write_nginx_snippets(rows, keep_urls, redirects)
    print(f"wrote {OUT}; removed {counts['301'] + counts['410']} HTML files")
    return 0


def write_nginx_snippets(rows: list[dict], keep_urls: set[str],
                         redirects: dict[str, str]) -> None:
    """Render all 301/410 nginx snippets from the consolidation rows.

    Sources already redirected by hand-maintained snippets are skipped to avoid
    duplicate exact locations. The generated snippets themselves must not count
    here, otherwise a re-render would drop every previously generated 301.
    """
    del redirects  # kept for call compatibility; see docstring
    external_redirect_sources = {
        source for source, target in old_redirects(exclude_generated=True).items()
