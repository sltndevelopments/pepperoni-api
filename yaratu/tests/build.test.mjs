...
test("agent discovery files exist without fake auth, MCP or commerce", async () => {
  ...
  const oauth = JSON.parse(await readFile(join(dist, ".well-known/oauth-authorization-server"), "utf8"));
  const oidc = JSON.parse(await readFile(join(dist, ".well-known/openid-configuration"), "utf8"));
  const resource = JSON.parse(await readFile(join(dist, ".well-known/oauth-protected-resource"), "utf8"));
  const mcp = JSON.parse(await readFile(join(dist, ".well-known/mcp/server-card.json"), "utf8"));
  const mcpAlias = JSON.parse(await readFile(join(dist, ".well-known/mcp.json"), "utf8"));
  const register = JSON.parse(await readFile(join(dist, "agents/register"), "utf8"));
  const authMd = await readFile(join(dist, "auth.md"), "utf8");
  ...
  assert.match(homeHtml, /navigator\.modelContext/);
  assert.match(homeHtml, /provideContext/);
  assert.match(homeHtml, /registerTool/);
  assert.match(homeHtml, /name: "list_products"/);
  ...
  assert.match(robots, /Allow: \/auth\.md/);
  ...
  assert.equal(mcp.transport.endpoint, "https://yaratu.com/mcp");
  ...
  assert.match(authMd, /oauth-protected-resource/);
  assert.doesNotMatch(authMd, /<[a-zA-Z]/);
  const built = await files(dist);
