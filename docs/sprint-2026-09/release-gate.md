...
## 9. Журнал 2026-09-12: тестовая заявка, доставка, коммерческие факты

**§6 п.5 — тестовая заявка (по разрешению владельца, телефон владельца, 15:06–16:40 UTC):**

| Шаг | Результат |
|---|---|
| Заявка №1 с `/sosiski-dlya-hotdog` | 🆔 `84a8dd86…` → **502 `delivery_failed`**; форма показала ошибку, `generate_lead` не отправлен (клиент вёл себя правильно). Причина измерена, не предположена: из 15 проб Bot API с VPS 4 таймаута dual-stack, 3 — форс-IPv4; TCP+TLS до `149.154.167.220` 70 мс → зависает ~25 % запросов |
| Фикс `a6c0c7a8f` | 3 попытки × 5 с (пауза 1/2 с), дисковый спул в `lead_dedup.sqlite`, ответ `queued:true` вместо 502, `/lead-health.queued`, cron `lead_intake_flush.py` каждые 2 мин (досылка + e-mail-алерт); `cbb4a03d3` — SMTP-переменные в `seo-agent.env` |
| Заявка №2 со страницы | 🆔 `03515b4efe2c`: 3 SKU в сообщении, `page_lang=ru`, ровно один `generate_lead` с `transaction_id=lead_id`; доставлено с первой попытки |
| Дедуп | 🆔 `3ac1f579e0ee` → повторный POST с тем же `client_ref` → `{"duplicate":true}`, второго сообщения в группе нет |
| Состояние | `/lead-health` → `{"configured":true,"ok":true,"queued":0}`; crontab flush = 1 строка; VPS HEAD = `origin/main` |
| Почта | `627cb27d0`: копия каждой заявки на `LEAD_EMAIL_TO=995620@gmail.com` из фонового потока; **выключена**, пока владелец не задаст секреты `ALERT_SMTP_USER`/`ALERT_SMTP_PASS` |

Известный компромисс: запрос, который дошёл до Telegram, но не вернул ответ за 5 с, будет повторён —
возможен дубль сообщения с тем же 🆔 в группе. Принято: дубль дешевле потерянной заявки, 🆔 позволяет
отличить.

**Коммерческие факты владельца → сайт (`13fb744ac`, `b7a063b8d`, `ec4c86f6c`):** минимальный тираж
СТМ «от 5 тонн» (класс из ~60 мест: страницы, JSON-LD, overrides, генераторы; новый блокирующий гейт
`check_stm_min_run.py` в `sync-vps.sh`); мощность 12 т/смену; паллетный базис EXW Казань; документы
«Честный знак», ЭТрН, ВСД; доставка/сроки/образцы — индивидуально. Источники: `brand.txt`,
`data/evidence_registry.json`.

**Контент (`05e47d26a`):** `/blog/tolko-halyal-kakie-kategorii-myasa` — ответ на самый частый вопрос
закупщика; `page_reviewer` на VPS: **pass** (первые два прогона — reject за отсутствие `tldr-answer` и
секции сценария применения; исправлена страница, не гейт). Кейс — не опубликован: нет фактов от продаж.

**Инфраструктура:** `partner-intake.service` (форма `/china`) был в crash-loop из-за прав на
`/var/www/pepperoni/.env` — исправлено и закреплено в `deploy-vps.yml` (`23f94ff25`).

**Открыто для человека:** секреты SMTP; профили (владелец сам); «паллета vs от 20 кг» на старых
страницах; факты для обезличенного кейса; блок технолога (KD-006/KD-007 в карантине).
