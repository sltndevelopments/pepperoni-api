...
- Падение кликов почти целиком — гео-страницы, снятые сознательно. Границы дат (GSC, выгрузка 2026-06-01…2026-09-06): гео-клики по полным месяцам июнь 270 · июль 342 · август 116; сентябрь **неполный** (1–6 сентября) — 10, с полными месяцами не сравнивается. Сопоставимые окна (1–6 число каждого месяца): всего 53 → 128 → 53 → 40, из них гео 37 → 87 → 29 → 10, не-гео 16 → 41 → 24 → 30. То есть за первые шесть дней сентября клики без гео выше, чем в те же дни августа; товары 5 → 5 → 0 → 1 — единственный ряд, за которым следить. Первый корректный помесячный сравнительный срез — после 30 сентября или по законченным ISO-неделям (`weekly-search-baseline.csv`).
...
Причина не ждать: 403 на discovery-файлах ломает интеграции (MCP, custom GPT, любой скрипт,
читающий каталог). Это техническая доступность, а не механизм «рекомендаций ИИ»: Google не
требует для своих AI-функций ничего сверх обычной индексации, OpenAI описывает доступ
OAI-SearchBot к страницам отдельно от API. Открытый API сам по себе не делает компанию
рекомендуемой — это делают проверяемые факты на страницах и упоминания вне сайта.
...
**2026-09-11:** деплой из `main` (после автокоммита SEO-агента 05:30) выполнил untracked-копию
скрипта с VPS и переустановил старый untracked сниппет → расширение откатилось. Переустановлено
из файлов ветки; окончательно чинится только merge (tracked-файлы затирают untracked при
`git reset --hard`). Подробно — `release-gate.md`.
