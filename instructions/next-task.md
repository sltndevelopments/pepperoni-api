...
- **2026-09-22 Блог: три статьи из каталога, темп 1/неделя, без автодобора.**
  ...
  - В индекс не добавлялись. Чтобы разрешить: вручную перевести URL в allowlist `scripts/build_index_manifest.py` и пересобрать sitemap.

- **2026-09-22 Скрытый дашборд эффективности завода /dashboard — арифметика KPI сверена.**
  ...
  - Метрики завода, один знаменатель на карточку:
    - Выработка на сотрудника: 869,1 млн / 110 = 7,9 млн ₽ (2025). План 2026 при том же штате: 9,1 млн (+15%). Ряд 6,5 → 9,8 → 13,6 снят: он не сходится со штатом 110.
    - Чел·час в деньгах (3 919 ₽/ч в 2025 г. → 4 509 ₽/ч план 2026).
    - Чел·час в объёме: 6,5 кг сняты с карточки. Это 120 т / (110 × 168 ч), весь штат, не цех. Цеховая цифра ждёт часы участка.
    - Съем с 1 м² цеха: 435 тыс. ₽/м² (2 000 м²). План цеха 2026: 500 тыс. Площадка 3 600 м²: 241 тыс. факт, 278 тыс. план.
    - Плотность роботизации 545 = 6 роботов / 110. Четыре контура проекта и Variovac/Bizerba — другой счёт.
    - Загрузка калькулятора: 1 смена (потолок 300 т) и 2 смены (потолок 600 т).
    - OData: страница не подключена к 1С. Пример — `AccumulationRegister_Продажи/Turnovers` и `Document_Табель`.
