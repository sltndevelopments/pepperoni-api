...
### Автономия (карантин, 2026-08-27)

Холодная автоотправка **выключена**. `auto_send_cold_email=false`. Живой SMTP на рынок закрыт в `channels.email.send_email` (класс исходящих, не одна функция): без `SALES_AGENT_ALLOW_LIVE_SEND=1` не уйдёт ни draft-pipeline, ни «тестовое письмо» из Telegram-бота. Письмо владельцу на `OWNER_EMAIL` — не рынок.

Не снимать карантин без legal review, policy и явного решения владельца.
